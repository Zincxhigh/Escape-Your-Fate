...
execute as @a[tag=is_monster] at @s run tp @s 218 88 -272
execute as @a[tag=tank] at @s run tp @s 263 80 -320
execute as @a[tag=reviver] at @s run tp @s 263 80 -320
execute as @a[tag=monster_detecter] at @s run tp @s 263 80 -320
title @a[tag=is_monster] actionbar {"text":"You are KEVIN (hint) {%^&$^^$kILL^%^I WANNA KILL}","color":"red","bold": true}
title @a[tag=tank] actionbar {"text":"You are leon (hint) Your way stronger and faster You know that right?","color":"white"}
title @a[tag=reviver] actionbar {"text":"You are ZACH (hint) You can help them walk again","color":"green"}
title @a[tag=monster_detecter] actionbar {"text":"You are MARIO (hint) You can sense unusual things near you:","color":"blue"}
